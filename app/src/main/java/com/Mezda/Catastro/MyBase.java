...
public class MyBase extends SQLiteOpenHelper {
    ...
    public static final String NOMBRE_BASEDATOS = "catprz.db";
    ...
    public void tuncatetables_MyBase(){
    ...
    public void TareasLocalesInexistentes(JSONArray response) throws JSONException {
        SQLiteDatabase database = getReadableDatabase();
        SQLiteDatabase database1 = getWritableDatabase();
        String[] columnas_recuperar = {"IdTareaP", "IdPredio", "ClaveCatast", "IdUsoPred", "UsoPredio", "Verif", "IdPersona", "VD", "Prop", "DirPro"};
        Cursor cursor = database.query("tareas", columnas_recuperar, null, null, null, null, null, null);
        
        cursor.moveToFirst();
        do {
            boolean existe=false;
            for (int i =0; i<response.length();i++){
                if (cursor.getString(2).equals(response.getJSONObject(i).getString("ClaveCT"))){
                    existe=true;
                }
            }
    
            if (!existe){
                    database1.delete("predios","IdPredio=?",new String[]{cursor.getString(1)});
                    database1.delete("levcampo","IdUsoPredio=?",new String[]{cursor.getString(3)});
                    database1.delete("tareas","ClaveCatast=?",new String[]{cursor.getString(2)});
            }
    ...
    public boolean TareaExistente(String ClaveCat) {
        ...
        cursor.moveToFirst();
        do {
            if (cursor.getString(2).equals(ClaveCat)) {
                return true;
            }
    ...
    public boolean TablaTareaVacia() {
        ...
        boolean empty = true;
        Cursor cur = database.rawQuery("SELECT COUNT(*) FROM tareas", null);
    ...
    public void InsertLevCons(String idpre,
        ...
        if (db != null) {
            ...
            valores.put("IdTipoCons", idtc);
            valores.put("IdTerminoCons", idtec);
            Log.e("LevCons", idpre + " " + idtc + " " + idec + " " + idtec + " " + area + " " + ant);
