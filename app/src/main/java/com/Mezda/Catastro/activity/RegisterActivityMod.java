...
public class RegisterActivityMod extends AppCompatActivity implements
    ...
    public static String ctipo,ctermino,cestado,antx,areax,letrax;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        area.getEditText().setText(areax);
        ...
        ant.getEditText().setText(antx);
        ant.getEditText().setEnabled(false);
        ...
        letra.getEditText().setText(letrax);
        ...
        for(int i=0;i<DataSpinn.TerminoCons().size();i++){
            if(ctermino.equals(DataSpinn.TerminoCons().get(i).getClave())){
                lsp3.getSpinner().setSelection(i);
                lsp3.getSpinner().setEnabled(false);
                lsp3.setClickable(false);
            }
        }

        for(int i=0;i<DataSpinn.TipoCons().size();i++){
            if(ctipo.equals(DataSpinn.TipoCons().get(i).getClave())){
                lsp2.getSpinner().setSelection(i);
                lsp2.getSpinner().setEnabled(false);
                lsp2.setClickable(false);
            }
        }

        for(int i=0;i<DataSpinn.EstadoCons().size();i++){
            if(cestado.equals(DataSpinn.EstadoCons().get(i).getClave())){
                lsp1.getSpinner().setSelection(i);
                lsp1.getSpinner().setEnabled(false);
                lsp1.setClickable(false);
            }
        }
 ...
 +                        te1.getId());*/
                ...
                DeleteDif(DetailViewFragment.IdPredio,letrax);
                ...
                Toast.makeText(getApplication(), "Informacion Guardada! " +DetailViewFragment.IdTareaCampo, Toast.LENGTH_LONG).show();
                onBackPressed();
            }
        });
    ...
    private void DeleteDif(String IdPred, String Let) {
        SQLiteDatabase db = bd.getReadableDatabase();
        db.delete("predios",  "Letra='" + Let + "' AND IdPredio="+IdPred, null);
        db.close();
    }
    ...
    private static void UpdateTareas() {
        ...
        db.update("tareas", valores, "IdTareaP=" + DetailViewFragment.IdTareaCampo, null);
        db.close();
    }
